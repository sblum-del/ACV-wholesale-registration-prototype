...
// What the SF Application record looks like per scenario
const RESUME_STATE: Record<string, {
  docSignReceived: boolean
  destination: View
}> = {
  r1: { docSignReceived: false, destination: 'dealership-info' },
  r2: { docSignReceived: false, destination: 'terms-of-service' },
  r3: { docSignReceived: false, destination: 'banking' },
  r4: { docSignReceived: false, destination: 'docusign-prompt-post-banking' },
  r5: { docSignReceived: true,  destination: 'resume-all-complete' },
  r6: { docSignReceived: true,  destination: 'resume-all-complete' },
}

export function Resume5MSelect({ setView, activeScenario, setDocSignStatus, onLogout }: Props) {
  const cfg = RESUME_STATE[activeScenario] ?? RESUME_STATE['r1']
  const [loading, setLoading] = useState(false)

  const handleResume = () => {
    setLoading(true)
    setDocSignStatus({
      lpoa: cfg.docSignReceived ? 'received' : 'pending',
      taxResale: cfg.docSignReceived ? 'received' : 'pending',
    })
    setTimeout(() => {
      setLoading(false)
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <ScreenLabel id="RESUME-3" name="Resume — Select Dealership" />
      ...
      <div className="max-w-3xl mx-auto w-full px-6 pt-10 pb-16">
        ...
        {/* In Progress dealership — clean, no checklist */}
